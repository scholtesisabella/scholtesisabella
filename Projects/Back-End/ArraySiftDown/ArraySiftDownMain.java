...
public final class ArraySiftDownMain {
    ...
    private static void siftDown(int[] array, int top, int last) {
        ...
        if (array.length > 1) {
            ...
            int lTree = 2 * top + 1;
            int rTree = lTree + 1;
            //checks to see if right child is the smaller child
            if (rTree <= last && array[lTree] >= array[rTree]) {

                //makes root the root of right subtree
                exchangeEntries(array, top, rTree);
                siftDown(array, rTree, last);
            } else {
                //makes root of heap the root of left subtree
                exchangeEntries(array, top, lTree);
                siftDown(array, lTree, last);
            }
        }
