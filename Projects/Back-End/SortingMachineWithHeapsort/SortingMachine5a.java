...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void exchangeEntries(T[] array, int i, int j) {
        ...
        //loop to swap entries in array
        if (i != j) {
            //saves original array value of i into temporary variable
            T tmp = array[i];
            //sets array value at i to array value at j
            array[i] = array[j];
            //sets array value at j to temporary variable (original value of i)
            array[j] = tmp;
        }
    ...
    private static <T> void siftDown(T[] array, int top, int last,
        ...
        if (array.length > 1) {
            //Initializes left and right children values
            int lTree = 2 * top + 1;
            int rTree = lTree + 1;
            //check if the right tree root is not the last value in the heap
            //and check if the left root is greater than the right root
            if (rTree <= last
                    && order.compare(array[lTree], array[rTree]) >= 0) {
                //compare the root of the tree to the right root
                if (order.compare(array[top], array[rTree]) >= 0) {
                    //if right root is smaller than the overall root, switch
                    //and sift down the entries
                    exchangeEntries(array, top, rTree);
                    siftDown(array, rTree, last, order);
                }
                //if the right root is the last value
                //or the left root isn't greater than the right root
            } else if (lTree <= last) {
                //compare the root of the tree to the left root
                if (order.compare(array[top], array[lTree]) >= 0) {
                    //if left root is greater than the overall root, switch
                    //and sift down the entries
                    exchangeEntries(array, top, lTree);
                    siftDown(array, lTree, last, order);
                }
            }
        }
    }
    ...
    private static <T> void heapify(T[] array, int top, Comparator<T> order) {
         ...
         */
        //setting indexes of left and right subtrees
        int l = 2 * top + 1;
        int r = l + 1;

        //makes heap out of right subtree
        if (r < array.length) {
            //recursive call
            heapify(array, r, order);
        }

        //makes heap out of left subtree
        if (l < array.length) {
            //recursive call
            heapify(array, l, order);
        }
        //sifts root down and makes array into heap
    ...
    private static <T> T[] buildHeap(Queue<T> q, Comparator<T> order) {
        ...
        //loads queue into heap
        for (int i = 0; i < heap.length; i++) {
            heap[i] = q.dequeue();
        }
        //creates a heap from the loaded heap
    ...
    private static <T> boolean isHeap(T[] array, int top, int last,
         ...
         */
        //checks structure of array to check if it is a heap recursively
        int left = 2 * top + 1;
        boolean isHeap = true;
        if (left <= last) {
    ...
    public final void changeToExtractionMode() {
        ...
        //shuts off insertion mode and builds heap from queue using order
    ...
    public final T removeFirst() {
        ...
        //saves first value to variable
        T removedVal = this.heap[0];

        //decrementing size of heap
        this.heapSize--;
        //swapping the first entry with the last entry of heap
        exchangeEntries(this.heap, 0, this.heapSize);

        //sifts down root and removes first value from heap
        siftDown(this.heap, 0, this.heapSize - 1, this.machineOrder);

        assert this.conventionHolds();

        //returns removed first value
        return removedVal;
    }
    ...
    public final boolean isInInsertionMode() {
        assert this.conventionHolds();
        //returns boolean value of if this is in insertion mode
    ...
    public final int size() {
        int size = 0;
        //if is not in insertion mode, size is heapSize
        if (!this.insertionMode) {
            size = this.heapSize;
        } else {
            //if it is, size is the length of the queue
